...
// Windows 9x hack to emulate ToUnicode().
// Not battle tested, but it seems to work!
// "wchars" is an array with enough space for one character and a null terminator.
int ToNonUnicode(UINT key, UINT scanCode, PBYTE keyboard, wchar *wchars)
{
	WORD mapped;
	ToAscii(key, scanCode, keyboard, &mapped, 0);
	char asChar = (char) mapped;
	int updated = mbstowcs(wchars, &asChar, 1);
	wchars[1] = L'\0';
	return updated;
}
...
win_key_down(WPARAM wParam, LPARAM lParam)
{
  ...
  if(wchars_n == 0)
  {
	  // ToUnicode does not work on Windows 9x.
	  // Fall back to a wrapper around ToAscii()
	  // As the name implies, this will not work for non-ascii key inputs.
	  wchars_n = ToNonUnicode(key, scancode, keyboard, wchars);
  }
